...
app.post('/friends/connect',function(req,res){
  ...
  var id=req.body.id;
  session
      .run("MATCH(a:Person {name:{nameParam1}}),(b:Person {name:{nameParam2}}) MERGE(a)-[r:FRIENDS]->(b) RETURN a,b",
...
app.post('/person/born/add',function(req,res){
  ...
  var id=req.body.id;
  session
      .run("MATCH(a:Person {name:{nameParam}}),(b:Location {city:{cityParam},state:{stateParam}}) MERGE(a)-[r:BORN_IN {year:{yearParam}}]->(b) RETURN a,b",
      {nameParam:name,cityParam:city,stateParam:state,yearParam:year})
      .then(function(result){
        if(id&&id!=null){
          res.redirect('/person/'+id);
        }else{
